...
```js
  var columnDefs = [
    setColumn(["useCheck", "선택", "center", 50], [true, ["check"], false, "", ""], false, false, false, ""),
    setColumn(["aiCode", "AI 코드", "center", 70, false]),
    setColumn(["aiName", "AI 명칭", "left", 200, false]),
    setColumn(["dataFormat", "데이터 형식", "left", 80, false]),
  ];
...
보통 그리드를 선언할 때는 위와 같이 선언하며 useCheck 컬럼 같은 경우 'Y' 또는 'N' 값을 수정한 후 gridHelper를 이용해 grid 데이터를 불러와도 useCheck의 변경된 값을 확인할 수 없다. (useCheck 컬럼 자체가 존재하지 않음) 따라서 DB에서 USE_CHECK 컬럼과 값을 전달해주어야 변경 사항이 컨트롤된다.

```sql
SELECT *, 'N' AS USE_CHECK
  FROM T_AI_INFO AI
 WHERE AI.AI_CODE NOT IN ('00', '01', '02');
```
...
<form:grid id="myGrid" height="460px" />
```
...
var columnDefs = [
	setColumn(
		["useCheck", "선택", "center", 50], [true, ["check"], false, "", ""], false, false, false, ""),
	setColumn(["aiCode", "AI 코드", "center", 70, false]),
	setColumn(["aiName", "AI 명칭", "left", 200, false]),
	setColumn(["dataFormat", "데이터 형식", "left", 80, false]),
];
...
function loadTableData(isMsg) {
	...
	//---- 그리드 조회 시 필수요소 start
	frm.addParam("result", $("#myGrid")); // 그리드 태그
	frm.addParam("grid", "gridOptions"); // 그리드 옵션은 위에서 정의한 옵션명을 따라가야한다. gridOptions.all 파라미터 추후 확인 필요
	//---- 그리드 조회 시 필수요소 end
	frm.addParam("func", "IQ");
	frm.addParam("query_id", "event.commissioning.select_aiList");
	frm.addParam("dataType", "json");
	if (isMsg === false) {
		frm.addParam("afterAction", false);
	}
...
### 읽기
\_gridHelper.getAllGridData

### 쓰기
\_gridHelper.setGridData(인덱스, 필드명(내부값), 설정할 값\[, 반영 여부 기본값 true])
```js
gridOptions.api.updateRowData({add:[data[0]]});
```

### 삭제

```js
gridOptions.api.applyTransaction({remove: removedRows});
```

```js
frm.addParam("grid", "gridOptions.all"); // 전부
frm.addParam("grid", "gridOptions") // 상태변경값이 공백이 아닌 것들만
```
...
{
  "hBarcode": "",
  "hInoutGlnCode": "8805901000073",
  "hDeliveryGlnCode": "",
  "inoutGlnText": "음극재 인공흑연(8805901000073)",
  "func": "IS",
  "dataType": "json",
  "grid": "gridOptions.all",
  "objId": "frmMain",
  "Token": "",
  "pageUrl": "/event/manufacturing.do",
  "userDeptName": "D00001",
  "userName": "관리자(WNP)",
  "deptName": "D00001",
  "userRoleIdAry": ["AD"],
  ...
  "userGlnCode": "8808456215001",
  "userRoleName": "총관리자",
  "userLangPack": "kr",
  "userRoleGubnNameAry": ["관리자"],
  "userRoleId": "AD",
  "userRoleGubnAry": ["A"],
  "userId": "admin",
  "userRoleNameAry": ["총관리자"],
  "userCompCode": "wnpsoft",
  "userPhone": "010-1111-1111",
  "userToken": "",
  "userConnectDb": "wnp_fw",
  "list": [
    {
      "index": 0,
      "grdBcId": "BC2025000460",
      "grdGtinCode": "8805903000040",
      "grdLotBatch": "20251104201306038298",
      "grdCodeTypeName": "SGTIN",
      "grdPrdtName": "대용량 배터리 BTS-1230 12V 300Ah",
      "grdPrdtTypeName": "배터리팩",
      "grdRowIndex": 0,
      "hBarcode": "",
      "hInoutGlnCode": "8805901000073",
      "hDeliveryGlnCode": "",
    ...
    {
      "index": 1,
      "grdBcId": "BC2025000459",
      "grdGtinCode": "8805901000103",
      "grdLotBatch": "20251104183336508180",
      "grdCodeTypeName": "SGTIN",
      "grdPrdtName": "전구체 알루미늄 첨가형 전구체",
      "grdPrdtTypeName": "전구체",
      "grdRowIndex": 1,
      "hBarcode": "",
      "hInoutGlnCode": "8805901000073",
